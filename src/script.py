...
def process_option(option, item, session):
    option_url = utils.ANALYSIS_URL.format(optionValue=option)
    response = session.get(option_url)
...
def process_consumer(item, session):
    item_url = utils.CONSUMER_URL.format(consumerValue=item)
    response = session.get(item_url)
    data = response.text 
    variable_line = re.search(f'var ${utils.VARIABLE_NAME} = \'(.*)\'', data)
    print(variable_line, utils.VARIABLE_NAME)
    if variable_line:
